...
const Notifications = () => {
            ...
            const newNotifications = [
                {
                    ...
                    detail:
                        "Anda memiliki janji temu dengan pasien Putri Selena hari ini pukul 10:00 pagi. Silakan siapkan hal yang diperlukan untuk sesi terapi.",
                    link: "/appointment/1",
                    color: "text-blue-500"
                },
                {
                    ...
                    detail:
                        "Janji temu dengan pasien Alice Johnson yang dijadwalkan pada 15 Mei 2024 pukul 11:00 telah dibatalkan. Mohon jadwalkan ulang janji temu sesuai ketersediaan Anda.",
                    link: "/appointment/2",
                    color: "text-red-500"
                },
                {
                    ...
                    detail:
                        "Catatan terbaru telah ditambahkan untuk pasien Putri Selena. Silakan periksa catatan tersebut dan tambahkan komentar atau rekomendasi Anda jika diperlukan.",
                    link: "/appointment/3",
    ...
    const fetprofile = async () => {
        const token = localStorage.getItem("iduserlogin")
        try {
            const response = await axios.get(`http://localhost:3001/appointmentsalls/${token}`, {
                headers: {
                ...
                <div className="bg-white rounded-sm shadow-md ring-1 ring-black ring-opacity-5 p-4 overflow-y-auto">
                    {data.length > 0 ? (
                        data.map((notification) => (
                            <div
                                key={notification.id}
                                className="py-2 flex justify-between items-center border-b border-gray-200 last:border-none"
                            >
                                <div className="flex items-center gap-2">
                                    <HiOutlineExclamationCircle className={`h-6 w-6 ${notification.color}`} />
                                    <div>
                                        <div className="font-bold">{notification.registration_date}</div>
                                        <a className="text-blue-500 hover:underline">
                                            Anda memiliki janji temu dengan dokter {notification.name} hari ini pukul{" "}
                                            {notification.registration_date} pagi. Silakan siapkan hal yang diperlukan
                                            untuk sesi terapi.
                                        </a>
